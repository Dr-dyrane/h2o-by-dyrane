...
const LEVEL_CLASSES = [
    ...
    "bg-emerald-500/60",
    "bg-emerald-400/90",
    "bg-emerald-400 shadow-[0_0_4px_rgba(52,211,153,0.28)]",
] as const;
...
const LEGEND_CLASSES = [
    ...
    "bg-emerald-500/60",
    "bg-emerald-400/90",
    "bg-emerald-400 shadow-[0_0_4px_rgba(52,211,153,0.24)]",
] as const;
...
export const ContributionGraph = () => {
    ...
    return (
        <TooltipProvider delayDuration={0}>
            <div className="w-full max-w-7xl mx-auto px-4 py-16 md:px-6 md:py-20">
            ...
            {/* Graph */}
            <div className="w-full overflow-x-auto pb-4 squircle p-4 md:p-5 surface-panel">
                <div className="min-w-[680px] md:min-w-[800px] flex gap-1">
                            ...
                            {week.map((day, dayIndex) => (
                                <Tooltip key={`${weekIndex}-${dayIndex}`}>
                                    <TooltipTrigger asChild>
                                        <div
                                            className={`w-2.5 h-2.5 md:w-3 md:h-3 squircle-chip transition-all duration-200 hover:scale-110 hover:z-10 cursor-crosshair ${LEVEL_CLASSES[day.level]}`}
                                        />
                                    </TooltipTrigger>
            ...
            <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-6">
                {stats.map(stat => (
                    <div key={stat.label} className="squircle-nav p-4 surface-card">
                        <div className="flex items-start justify-between mb-1">
                            <div className="text-2xl text-[var(--text)] font-light tracking-tight tabular-nums">
                                {stat.value}
                            </div>
                            {stat.live && (
                                <div className="mt-1.5 h-1.5 w-1.5 squircle-pill bg-emerald-500/80" />
                            )}
                        </div>
                        <div className="text-[var(--text-muted)] text-[10px] uppercase tracking-[0.12em] font-mono">
                            {stat.label}
                        </div>
                    </div>
                ))}
