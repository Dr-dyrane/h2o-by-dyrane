...
interface TeardownPoint {
  icon: string;
  label: string;
  detail: string;
}

interface UITeardownProps {
    ...
    <div className={`grid gap-3 ${className}`}>
      {points.map((point, idx) => {
        ...
        return (
          ...
          >
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--surface-elevated)] text-[var(--cat-ux)] transition-transform duration-300 group-hover:scale-110">
              <Icon size={18} />
            </div>
